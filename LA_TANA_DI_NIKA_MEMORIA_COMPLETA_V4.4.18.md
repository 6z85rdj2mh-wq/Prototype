...
Versione di riferimento: **V4.4.16**  
Data: **28 luglio 2026**
...
## Aggiornamento V4.4.16 — Guide gratuite CMS-ready

La V4.4.16 parte dalla V4.4.15 e non ridisegna le Guide gratuite. Trasforma il loro archivio in una struttura riproponibile dalla futura dashboard.

### Regola pubblica

Le pagine pubbliche mostrano esclusivamente guide con:

- `status: "published"`;
- `deletedAt: null`.

Bozze, guide archiviate e contenuti nel cestino non devono essere visibili né nell’overview né nel dettaglio.

### Ciclo di vita

- `draft`: salvabile anche se incompleta;
- `published`: visibile al pubblico;
- `archived`: conservata ma nascosta;
- `trash`: nel cestino e ripristinabile;
- eliminazione definitiva: richiede una seconda conferma.

### Creazione e duplicazione

La futura admin deve permettere:

- nuova guida da template vuoto;
- duplicazione di qualsiasi guida;
- modifica completa di testi e immagini;
- attivazione/disattivazione dei moduli;
- duplicazione e riordinamento dei moduli;
- anteprima prima della pubblicazione;
- pubblicazione, archivio, cestino e ripristino.

Non deve essere necessario creare o modificare file HTML, CSS o JavaScript per aggiungere un nuovo mazzo.

### Moduli Guide gratuite

Obbligatori nel record:

- `why`;
- `keyCards`;
- `decklist`.

Opzionali e riutilizzabili:
...
- massimo 5 elementi nelle sezioni con carte;
- massimo 5 istanze per tipologia opzionale;
- ID e slug univoci.

### File di riferimento

- `js/data/free-guides.js`;
- `admin/free-guides/free-guide-template.json`;
- `admin/free-guides/free-guides-admin-contract.json`;
- `admin/free-guides/ADMIN_WORKFLOW.md`;
- `SUPABASE_FREE_GUIDES_SCHEMA_V4.4.16.sql`;
- `COME_AGGIUNGERE_UNA_GUIDA_GRATUITA_V4.4.16.md`.

### Stato reale

La struttura dati e le API sono pronte. La schermata grafica admin e il collegamento Supabase non sono ancora implementati. Il prossimo macro-passo editoriale è rendere CMS-ready Articoli e report.
...
# Aggiornamento V4.4.18 — Articoli e report CMS-ready

La V4.4.18 è la base stabile successiva alla V4.4.16. La V4.4.17 è stata saltata intenzionalmente su richiesta dell’utente.

## Articoli e report
- La pagina `/editoriale/articoli/` non è più un’impalcatura.
- La libreria mostra solo articoli pubblicati, con ricerca, filtri e contenuto in evidenza.
- Il dettaglio usa una sola pagina generica: `/editoriale/articoli/articolo/?id=slug`.
- Stati supportati: draft, scheduled, published, archived, trash.
- Azioni admin predisposte: crea, duplica, modifica, carica media, riordina moduli, attiva/disattiva, anteprima, programma, pubblica, archivia, cestina, ripristina, elimina definitivamente.
- Moduli disponibili: text, image, quote, keyPoints, stats, results, gallery, video, related, callout.
- Almeno un modulo testo attivo è obbligatorio; massimo cinque istanze per tipologia.
- La dashboard grafica e Supabase reale non sono ancora implementati.

## Architettura media
- `js/config/services.js` centralizza configurazioni non segrete.
- `js/services/media-service.js` risolve immagini locali oggi e predisposte per Supabase Storage/R2.
- Nessuna chiave segreta deve mai entrare nel frontend.
- Supabase resta il cervello e conterrà dati/utenti/permessi.
- Cloudflare R2 sarà l’archivio immagini futuro.
- Twitch è il provider video corrente; titolo, descrizione, thumbnail, tag e riferimento saranno salvati in Supabase.
- La struttura video resta astratta per poter migrare in futuro.

## Prossimo macro-step
Dopo approvazione e caricamento della V4.4.18: uniformare i contratti di Guide gratuite, Guide premium e Articoli nel layer dati condiviso, quindi iniziare la dashboard admin unica solo dopo aver definito le restanti sezioni pubbliche modificabili.
